...
do
scp -q -p /home/wasadmin/maintence/scripts/local/wlpStatus.sh wasadmin@$copyhost:/opt/liberty/maintence/scripts/
done
########################################################
echo "Connecting the DBB preprod servers to execute "wlpStatus.sh" script..."
$DBBpreprodServerlist
echo "wating few seconds, start execution..."
for host in $(cat /home/wasadmin/maintence/scripts/properties/DBB-server-list-PREPROD.txt);
do
echo ""$host""
ssh -q wasadmin@$host "sh /opt/liberty/maintence/scripts/wlpStatus.sh" | grep "process ID"
