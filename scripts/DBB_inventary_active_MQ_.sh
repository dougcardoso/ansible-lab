...
echo "# script name: DBB_inventary_ActiveMQ.sh #"
echo "# run ./DBB_inventary_ActiveMQ.sh #"
...
touch $TEMPscript_path/DBB_INVENTARY_ActiveMQ.log
chmod -R 755 $TEMPscript_path
truncate -s0 $TEMPscript_path/DBB_INVENTARY_ActiveMQ.log
########################################################
...
do
ssh -q wasadmin@$host "hostname -f ; hostname -i ; echo "check if active MQ exist:" ; ls /opt/liberty/apache-activemq-5.13.3 ; echo "check if activeMQ is running:" ; sudo /opt/liberty/apache-activemq-5.13.3/bin/activemq status" >> $TEMPscript_path/DBB_INVENTARY_ActiveMQ.log 
echo "=====================================" >> $TEMPscript_path/DBB_INVENTARY_ActiveMQ.log
echo "script processing... "$host""
done
########################################################
#cat $TEMPscript_path/DBB_INVENTARY_SERVER.log
echo "Log path: "$TEMPscript_path"/DBB_INVENTARY_ActiveMQ.log"
cat $TEMPscript_path/DBB_INVENTARY_ActiveMQ.log
echo "Script done!"
