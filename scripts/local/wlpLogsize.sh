...
do
	echo "Running check "$libertyINSTANCEServer"";
		printf $GREEN
			ps -ef | grep $libertyINSTANCEServer > /tmp/$libertyINSTANCEServer.wlpLogsize.log ;
		printf $NORMAL
...
#		ls -ltr /etc/init.d/wlp* | grep "wlp";
	echo " ===>>> STATUS <<<==="
		...
		ls -ltr /opt/liberty/wlp/usr/servers/$libertyINSTANCEServer/logs/;
		printf $NORMAL
	echo " ===>>> STATUS <<<==="
done
printf $NORMAL
