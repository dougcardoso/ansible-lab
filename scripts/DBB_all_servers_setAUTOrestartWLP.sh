...
do
scp -p local/setAUTOrestartWLP.sh wasadmin@$copyhost:/opt/liberty/maintence/scripts/
done
########################################################
echo "############################################"
...
do
ssh wasadmin@$host "sh /opt/liberty/maintence/scripts/setAUTOrestartWLP.sh ; hostname -f" > $TEMPscript_path/$host.setAUTOrestartWLP.log
done
echo "Script done"
