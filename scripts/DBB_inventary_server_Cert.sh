...
touch $TEMPscript_path/DBB_INVENTARY_SERVER_CERTS.log
chmod -R 755 $TEMPscript_path
truncate -s0 $TEMPscript_path/DBB_INVENTARY_SERVER_CERTS.log
########################################################
...
do
ssh -q wasadmin@$host "hostname -f ; hostname -i ; echo "liberty:" ; ls /opt/liberty/wlp/usr/servers ; echo "key certs directory:" ;ls -l /opt/liberty/keys ; echo "key file content:" ; cat /opt/liberty/keys/key.xml; echo "cksum trust file:"; cksum /opt/liberty/keys/trust.jks " >> $TEMPscript_path/DBB_INVENTARY_SERVER_CERTS.log
echo "=====================================" >> $TEMPscript_path/DBB_INVENTARY_SERVER_CERTS.log
echo "script processing... "$host""
done
########################################################
#cat $TEMPscript_path/DBB_INVENTARY_SERVER.log
echo "Log path: "$TEMPscript_path"/DBB_INVENTARY_SERVER_CERTS.log"
echo "Script done!"
